...
export async function loadGameState(userId: string): Promise<GameStateDB | null> {
...
export async function loadLessonProgress(userId: string): Promise<LessonProgressDB[]> {
  const { data, error } = await supabase
...
export async function saveLessonProgress(progress: LessonProgressDB): Promise<boolean> {
  const { error } = await supabase
    .from("lesson_progress")
    .upsert(
      {
...
export async function loadTopicProgress(userId: string): Promise<TopicProgressDB[]> {
  const { data, error } = await supabase
    .from("topic_progress")
    .select("*")
    .eq("user_id", userId);
...
export async function saveTopicProgress(progress: TopicProgressDB): Promise<boolean> {
  const { error } = await supabase
    .from("topic_progress")
    .upsert(
      {
...
export async function addXPHistory(xp: Omit<XPHistoryDB, 'id' | 'created_at'>): Promise<boolean> {
  const { error } = await supabase
    .from("xp_history")
    .insert({
      user_id: xp.user_id,
